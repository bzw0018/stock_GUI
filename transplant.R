...
invariant_checker <- function(vector){
  ...
  if(length(vector_sort)>1){
  if(is.na(tail(vector_sort,1)) & is.na(head(vector_sort,1))){return("Two")}
...
reading <- function() {
  ...
  mac<-readline(prompt="Are you using Mac? (Yes: Y/No: N): ")
  organ<-readline(prompt="Which organ is it? (Heart: 1/Lung: 2/Liver: 3/Kidney: 4/Pancreas: 5/Intestin: 6): ")
  organ<-as.numeric(organ)
  if(organ==1){organ<-"heart"}
  if(organ==2){organ<-"lung"}
  if(organ==3){organ<-"liver"}
  if(organ==4){organ<-"kidney"}
  if(organ==5){organ<-"pancreas"}
  if(organ==6){organ<-"intestin"}
  NA_cleaning<-readline(prompt="The NA cells are already cleaned? (Yes: Y/No: N): ")
  Multi_Bino<-readline(prompt="Is it a Multinomial Analysis? (Yes: Y/No: N): ")
  Multi_Bino<-toupper(Multi_Bino)
  if(Multi_Bino=="Y"){
    category_no<-readline(prompt="How many categories for your Multinomial Analysis?: ")
    category_no<-as.numeric(category_no)
    g1<-0
    g2<-0
    g3<-0
    g4<-0
    for (i in 1:(category_no-1)){
      group_bin<-readline(prompt=paste("enter bin limit no.",i,": ",sep = ""))
      
      assign(as.character(paste(c("g",i),collapse = "")), group_bin)
    }
    g1<-as.numeric(g1)*365
    g2<-as.numeric(g2)*365
    g3<-as.numeric(g3)*365
  ...
  if(Multi_Bino=="N"){
    Duration_target<-readline(prompt="If it is Binomial what is the target's year? (e.g.: 1-Month: 1/12, 1-year: 1, 3 & half year : 3.5,...): ")
    Duration_target<-eval(parse(text=Duration_target))}
  
  FS_Method<-readline(prompt="Feature Selection Method? (RF: 1/Lasso-RF: 2/Fast Selection: 3/itself: 4/Literature: 5): ")  

  FS_Method_1=regexpr(pattern ='1',FS_Method)[1]
  FS_Method_2=regexpr(pattern ='2',FS_Method)[1]
  FS_Method_3=regexpr(pattern ='3',FS_Method)[1]
  FS_Method_4=regexpr(pattern ='4',FS_Method)[1]
  FS_Method_5=regexpr(pattern ='5',FS_Method)[1]
  
  if(FS_Method_1>0){
    RF_run<-readline(prompt="Do you already have the results of Random Forest? (Yes: Y/No: N): ")}
  if(FS_Method_2>0){  
    Lasso_run<-readline(prompt="Do you already have the results of Lasso? (Yes: Y/No: N): ")}
  if(FS_Method_3>0){  
    FCBF_run<-readline(prompt="Do you already have the results of Fast Feature Selection? (Yes: Y/No: N): ")}  
  if(FS_Method_4>0){  
    itself_run<-readline(prompt="Do you already have the results of the Itself? (Yes: Y/No: N): ")
  }
  if(FS_Method_5>0){  
    lit_run<-readline(prompt="Enter the variables you found in the literature seperated with comma? ")
  }
  conso_feat<-readline(prompt="Do you want to consolidate the features? (Yes: Y/No: N): ")
  conso_feat<-toupper(conso_feat)
  Multi_Bino<-readline(prompt="Is it a Multinomial Analysis? (Yes: Y/No: N): ")
  Multi_Bino<-toupper(Multi_Bino)
  
  n_folds<-readline(prompt="How many CV folds over data? (Enter a number): ")
  balanc_proc<-readline(prompt="You may consider applying Sample Balancing Algorithms (SMOTE/RUS/Hybrid)? (Yes: Y/No: N): ")
  balanc_proc<-toupper(balanc_proc)
  if(balanc_proc=="Y"){
    B_alg<-readline(prompt="Which Algorithm, you are interested? (SMOTE: 1/RUS: 2/Hybrid: 3/Nothing: 4 ): ")
  }
  B_alg<-as.numeric(B_alg)
  if(B_alg==3){
    H_por_low<-readline(prompt="Proportion of differences that you want to increase the minority? (type a proportion e.g. 0.5 for half): ")
    H_por_low<-as.numeric(H_por_low)
    H_por_high<-readline(prompt="Identify multiplication for size of smoted minority as the size of majority? (type 1.0 for no differences): ")
    H_por_high<-as.numeric(H_por_high)
  }
  
  mac<-toupper(mac)
  NA_cleaning<-toupper(NA_cleaning)
  
  RF_run<-toupper(RF_run)
  Lasso_run<-toupper(Lasso_run)
  FCBF_run<-toupper(FCBF_run)
  n_folds<-as.numeric(n_folds)
  balanc_proc<-toupper(balanc_proc)
  itself_run<-toupper(itself_run)
  lit_run<-toupper(lit_run)
  
  
  parameters<-c(mac,NA_cleaning,Multi_Bino,
                FS_Method,Lasso_run,FCBF_run,itself_run,n_folds,balanc_proc,B_alg,H_por_low,
                H_por_high,RF_run,FS_Method_1,FS_Method_2,FS_Method_3,FS_Method_4,FS_Method_5,lit_run,
                category_no,g1,g2,g3,g4,conso_feat,Duration_target,organ)
  
  parameters<-as.data.frame(parameters)
  parameters[]<- lapply(parameters, as.character)
  row.names(parameters)<-c("mac","NA_cleaning","Multi_Bino",
                           "FS_Method","Lasso_run","FCBF_run","itself_run","n_folds","balanc_proc","B_alg","H_por_low",
                           "H_por_high","RF_run","FS_Method_1","FS_Method_2","FS_Method_3","FS_Method_4","FS_Method_5",
                           "lit_run","category_no","g1","g2","g3","g4","conso_feat","Duration_target","organ")
                           
  names(parameters)<-c("content")
  input_object<-list()
  input_object$parameters<-parameters
  return(input_object)
}
...
initialization<-function(input_object){
  mac<-input_object$parameters["mac","content"] 
  NA_cleaning<-input_object$parameters["NA_cleaning","content"] 
  Multi_Bino<-input_object$parameters["Multi_Bino","content"]

  FS_Method<-input_object$parameters["FS_Method","content"]
  ...
  H_por_low<-input_object$parameters["H_por_low","content"]
  H_por_low<-as.numeric(H_por_low)
  
  H_por_high<-input_object$parameters["H_por_high","content"]
  H_por_high<-as.numeric(H_por_high)
  
  RF_run<-input_object$parameters["RF_run","content"]
  organ<-input_object$parameters["organ","content"]
  
  FS_Method_1<-as.numeric(input_object$parameters["FS_Method_1","content"])
  FS_Method_2<-as.numeric(input_object$parameters["FS_Method_2","content"])
  FS_Method_3<-as.numeric(input_object$parameters["FS_Method_3","content"])
  FS_Method_4<-as.numeric(input_object$parameters["FS_Method_4","content"])
  FS_Method_5<-as.numeric(input_object$parameters["FS_Method_5","content"])
 
  conso_feat<-input_object$parameters["conso_feat","content"]
  Duration_target<-as.numeric(input_object$parameters["Duration_target","content"])
  
#__________________________________Path Initialization_____________________________
if(mac=="Y"){root_folder<-"/Users/hamid/Dropbox/My R codes/Ali_Hamid/3rd_paper/data/"}else{root_folder<-"C:/Users/hza0020/Dropbox/My R codes/Ali_Hamid/3rd_paper/data/"}
  if(mac=="Y"){analysis_folder<-"/Users/hamid/Dropbox/My R codes/Ali_Hamid/3rd_paper/data/analysis/"}else{analysis_folder<-"C:/Users/hza0020/Dropbox/My R codes/Ali_Hamid/3rd_paper/data/analysis/"}
  target_time<-paste(round(Duration_target,3),"_year",collapse = "")
  if(Multi_Bino=="Y"){is_multi<-"Mult"
  target_time<-""}else{is_multi<-""}
  file_name<-paste(organ,".csv",sep="")
  var_names<-paste(organ,".csv",sep="")
...
itself_Method<-NULL
lit_Method<-NULL
if(FS_Method_1>0){RF_Method<-"RF"}
if(FS_Method_2>0){Lasso_Method<-"Lasso"}
if(FS_Method_3>0){FFS_Method<-"FFS"}
if(FS_Method_4>0){itself_Method<-"itself"}
if(FS_Method_5>0){lit_Method<-"Lit"}

method_fs<-paste(RF_Method,Lasso_Method,FFS_Method,itself_Method,lit_Method,sep="")
if(conso_feat=="Y"){consolidate<-"consolidated"}else{consolidate<-""}

if(balanc_proc=="Y"){
  if(B_alg==1){is_bal<-"Smote"}
  if(B_alg==2){is_bal<-"RUS"}
  if(B_alg==3){is_bal<-"Hybrid"}
  }else{is_bal<-"No-balancing"}
if(itself_run=="Y"){itself_run<-"itself"}else{itself_run<-""}

Random_Forrest_Location<-paste(c(root_folder,"Random_Forrest_",is_multi,"_variables.csv"),collapse = "")
Lasso_location<-paste(c(root_folder,"Lasso_",is_multi,"_variables.csv"),collapse = "")
Fast_Feature_Location<-paste(c(root_folder,"Fast_Feature_",is_multi,"_variables.csv"),collapse = "")
Literature_file<-paste(c(root_folder,"Literature_","_Litrature_Review.csv"),collapse = "")
consolidate_Location<-paste(c(root_folder,"consolidate_",is_multi,"_variables.csv"),collapse = "")

base_file<-paste(c(root_folder,file_name),collapse = "")
var_file<-paste(c(root_folder,var_names),collapse = "")
NA_free_file<-paste(c(root_folder,"clean_UnosData_",".csv"),collapse = "")
Out_put_file_name<-paste(c("_",method_fs,"_",consolidate,"_",is_bal,"_",is_multi,".txt"),collapse = "")
Out_put_file<-paste(c(analysis_folder,"_",method_fs,"_",consolidate,"_",is_bal,"_",is_multi,".txt"),collapse = "")

prediction_data<-paste(c(root_folder,"prediction_data","_",method_fs,"_",consolidate,"_","_",is_multi,"_",is_bal,"_",itself_run,"2004.csv"),collapse = "")
old_papers_vars<-paste(c(root_folder,"vars/","vars_57.csv"),collapse = "")
if(FS_Method_5>0){Literature_file<-old_papers_vars}

log_imp_folds_file<-paste(c(analysis_folder,"importance/","log_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")
svm_imp_folds_file<-paste(c(analysis_folder,"importance/","svm_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")
CRT_imp_folds_file<-paste(c(analysis_folder,"importance/","CRT_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")
NNET_imp_folds_file<-paste(c(analysis_folder,"importance/","NNET_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")
tan_imp_folds_file<-paste(c(analysis_folder,"importance/","tan_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")

log_perf_folds_file<-paste(c(analysis_folder,"performance/","log_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")
svm_perf_folds_file<-paste(c(analysis_folder,"performance/","svm_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")
CRT_perf_folds_file<-paste(c(analysis_folder,"performance/","CRT_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")
NNET_perf_folds_file<-paste(c(analysis_folder,"performance/","NNET_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")
tan_perf_folds_file<-paste(c(analysis_folder,"performance/","tan_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,".csv"),collapse = "")


Avg_log_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_log_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,"_",itself_run,".csv"),collapse = "")
Avg_svm_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_svm_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,"_",itself_run,".csv"),collapse = "")
Avg_CRT_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_CRT_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,"_",itself_run,".csv"),collapse = "")
Avg_NNET_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_NNET_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,"_",itself_run,".csv"),collapse = "")
Avg_tan_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_tan_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_",is_bal,"_",itself_run,".csv"),collapse = "")

itself_svm_Location<-paste(c(root_folder,"itself_svm_",is_bal,"_",is_multi,"_variables.csv"),collapse = "")
itself_log_Location<-paste(c(root_folder,"itself_log_",is_bal,"_",is_multi,"_variables.csv"),collapse = "")
itself_CRT_Location<-paste(c(root_folder,"itself_CRT_",is_bal,"_",is_multi,"_variables.csv"),collapse = "")
itself_NNET_Location<-paste(c(root_folder,"itself_NNET_",is_bal,"_",is_multi,"_variables.csv"),collapse = "")
itself_tan_Location<-paste(c(root_folder,"itself_tan_",is_bal,"_",is_multi,"_variables.csv"),collapse = "")

#_______________________________Output setup________________________________________

if(Multi_Bino=="Y"){Method_output<-"Multinomial"}else{Method_output<-"Binomial"}
FS_Method_output<-""
if(FS_Method_1>0){FS_Method_output<-paste("Random Forest",FS_Method_output,sep="")}
if(FS_Method_2>0){FS_Method_output<-paste("Lasso",FS_Method_output,sep=" & ")}
if(FS_Method_3>0){FS_Method_output<-paste("Fast Feature Selection",FS_Method_output,sep=" & ")}
if(FS_Method_4>0){FS_Method_output<-paste("Same Machine Learning Algorithm for Prediction and Feature Selection",FS_Method_output,sep=" & ")}
if(FS_Method_5>0){FS_Method_output<-paste("Literature",FS_Method_output,sep=" & ")}

 settings<-c(root_folder,analysis_folder,is_multi,file_name,var_names,
             method_fs,is_bal,Random_Forrest_Location,Lasso_location,consolidate_Location,
             Fast_Feature_Location,base_file,var_file,NA_free_file,Out_put_file,
             Literature_file,prediction_data,old_papers_vars,consolidate,
 ...
 row.names(settings)<-c("root_folder","analysis_folder","is_multi","file_name","var_names",
                        "method_fs","is_bal","Random_Forrest_Location","Lasso_location","consolidate_Location",
                        "Fast_Feature_Location","base_file","var_file","NA_free_file","Out_put_file",
                        "Literature_file","prediction_data","old_papers_vars","consolidate",
                        "log_imp_folds_file","svm_imp_folds_file","CRT_imp_folds_file","NNET_imp_folds_file","tan_imp_folds_file",
...
#_______________________________Required Libraries_______________________________________________________________________________________
  ...
  library(arules); library(DMwR); library(ROSE);library(taRifx); if(input_object$parameters["mac","content"]=="Y"){library(doMC)}
  set.seed(110)
...
Data<- read.csv(input_object$settings["base_file","content"] ,header =T, na.strings=c(""," ",".","NA","U")) 
cat("data is entered... \n")
names(Data)<-toupper(names(Data))

cat("variables are entered... \n")
#some changes happened in next 2 lines

VarsData <- read.xls(input_object$settings["var_file","content"], sheet =1)  
cat("variables are entered... \n")
VarsData_Phase <- VarsData[,c(1,4,7,9)]
VarsData_Phase<-VarsData_Phase[-1,]
names(VarsData_Phase)<-c("variables","phase","yearin","yearout")

S1 <- as.character(subset(VarsData_Phase, VarsData_Phase[,2] == "A")[,1])   #Stage 1
S2 <- as.character(subset(VarsData_Phase, VarsData_Phase[,2] == "A" | VarsData_Phase[,2] == "B")[,1])  #Stage 2
S3 <- as.character(subset(VarsData_Phase, 
                          VarsData_Phase[,2] == "A" | VarsData_Phase[,2] == "B" | VarsData_Phase[,2] == "C")[,1])  #Stage 3
S4 <- as.character(subset(VarsData_Phase, 
                          VarsData_Phase[,2] == "A" | VarsData_Phase[,2] == "B" | VarsData_Phase[,2] == "C"|
                            VarsData_Phase[,2] == "D")[,1])  #Stage 4
Spost<-as.character(subset(VarsData_Phase, VarsData_Phase[,2] == "D")[,1]) #just post transplants
S0 <- as.character(subset(VarsData_Phase, VarsData_Phase[,2] == "Z")[,1])
STarget <- as.character(subset(VarsData_Phase, VarsData_Phase[,2] == "TARGET")[,1])  # Target

#______________Excluding unwanted and post variables_______________________________________

Data <- Data[, -which(names(Data) %in% S0)] # Delete Z
Data <- Data[, -which(names(Data) %in% Spost)] # Delete post transplant
...
if(input_object$parameters["NA_cleaning","content"]=="N"){
  cat("data NA cleaning process is started... \n")
  
  for(i in 1:100000){
    cat("total number of the remained columns is:...",ncol(Try_data),"\n")
    cat("total number of the remained rows is:...",nrow(Try_data),"\n")
  ...
  write.csv(Try_data,input_object$settings["NA_free_file","content"])}else{Try_data<-read.csv(input_object$settings["NA_free_file","content"])}
...
if(as.numeric(input_object$parameters["FS_Method_1","content"]>0)){
  if(input_object$parameters["RF_run","content"]=="N"){
    library(Boruta)
...
Lit<-function(input_object){
  vars<-input_object$parameters["lit_run","content"]
  vars<-as.data.frame(unlist(strsplit(vars, split=",")))
  names(vars)<-"variables"
  input_object$lit<-vars
  return(input_object)
}
#=============================================================
