...
reading <- function() {
  ...
  stk<-readline(prompt="Which Stock? (AAPL:1, GOOG:2, CSCO:3, UAL:4 ): ")
  stk<-as.numeric(stk)
  if(stk==1){stk_comp<-"N_AAPL"}
  if(stk==2){stk_comp<-"N_GOOG"}
  if(stk==3){stk_comp<-"N_CSCO"}
  if(stk==4){stk_comp<-"N_UAL"}
  targ<-readline(prompt="Which Target No.? (1,2,3,4,5): ")
  targ<-as.numeric(targ)
  if(targ==1){trg<-"TARGET1"}
  if(targ==2){trg<-"TARGET2"}
  if(targ==3){trg<-"TARGET3"}
  if(targ==4){trg<-"TARGET4"}
  if(targ==5){trg<-"TARGET5"}
  ...
  tuning<-readline(prompt="Do you want to tune the parameters if the Machine Learning Algorithms? (Yes: Y/No: N): ")
  ...
  if(balanc_proc=="Y"){
    B_alg<-readline(prompt="Which Algorithm, you are interested? (SMOTE: 1/RUS: 2/Hybrid: 3): ")
  }
  ...
  parameters<-c(mac,NA_cleaning,Multi_Bino,
                ...
                category_no,g1,g2,g3,g4,stk_comp,trg,conso_feat,tuning)
  ...
  row.names(parameters)<-c("mac","NA_cleaning","Multi_Bino",
                           ...
                           "lit_run","category_no","g1","g2","g3","g4","stk_comp","trg","conso_feat","tuning")
...
initialization<-function(input_object){
  ...
  trg<-input_object$parameters["trg","content"]
  stk_comp<-input_object$parameters["stk_comp","content"]
...
if(mac=="Y"){root_folder<-"/Users/hamid/Dropbox/My R codes/Bin-Hamid/Stock_Prob/data/"}else{root_folder<-"C:/Users/hza0020/Dropbox/My R codes/Bin-Hamid/Stock_Prob/data/"}
if(mac=="Y"){analysis_folder<-"/Users/hamid/Dropbox/My R codes/Bin-Hamid/Stock_Prob/analysis/"}else{analysis_folder<-"C:/Users/hza0020/Dropbox/My R codes/Bin-Hamid/Stock_Prob/analysis/"}

  # if(is.null(parameters$root_folder)){root_folder<-getwd()}else{root_folder<-parameters$root_folder}
  # analysis_folder<-paste(root_folder,"/analysis/",sep="")
  root_folder<-"gh"
  analysis_folder<-"sfgs" 
  
if(Multi_Bino=="Y"){is_multi<-"Mult"}else{is_multi<-""}
file_name<-paste(stk_comp,".csv",sep = "") 
var_names<-"variables.xls"
...
tuning<-""
if(input_object$parameters["tuning","content"]=="Y"){tuning<-"tuned"}
...
Random_Forrest_Location<-paste(c(root_folder,"Random_Forrest_",is_multi,stk_comp,trg,"_variables.csv"),collapse = "")
Lasso_location<-paste(c(root_folder,"Lasso_",is_multi,stk_comp,trg,"_variables.csv"),collapse = "")
Fast_Feature_Location<-paste(c(root_folder,"Fast_Feature_",is_multi,stk_comp,trg,"_variables.csv"),collapse = "")
Literature_file<-paste(c(root_folder,"Literature_",stk_comp,trg,"_Litrature_Review.csv"),collapse = "")
consolidate_Location<-paste(c(root_folder,"consolidate_",is_multi,stk_comp,trg,"_variables.csv"),collapse = "")

base_file<-paste(c(root_folder,file_name),collapse = "")
var_file<-paste(c(root_folder,var_names),collapse = "")
NA_free_file<-paste(c(root_folder,"data_UnosData_",stk_comp,trg,"2004.csv"),collapse = "")
Out_put_file_name<-paste(c(stk_comp,"_",trg,"_",method_fs,"_",consolidate,"_",is_bal,"_",is_multi,"_",tuning,".txt"),collapse = "")
Out_put_file<-paste(c(analysis_folder,stk_comp,"_",trg,"_",method_fs,"_",consolidate,"_",is_bal,"_",is_multi,"_",tuning,".txt"),collapse = "")

prediction_data<-paste(c(root_folder,"prediction_data","_","_",method_fs,"_",consolidate,"_","_",is_multi,"_",is_bal,"_",itself_run,stk_comp,trg,"_",tuning,"2004.csv"),collapse = "")
old_papers_vars<-paste(c(root_folder,"vars/","vars_57.csv"),collapse = "")
if(FS_Method_5>0){Literature_file<-old_papers_vars}

log_imp_folds_file<-paste(c(analysis_folder,"importance/","log_imp_vars_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",stk_comp,trg,"_",tuning,".csv"),collapse = "")
svm_imp_folds_file<-paste(c(analysis_folder,"importance/","svm_imp_vars_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",stk_comp,trg,"_",tuning,".csv"),collapse = "")
CRT_imp_folds_file<-paste(c(analysis_folder,"importance/","CRT_imp_vars_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",stk_comp,trg,"_",tuning,".csv"),collapse = "")
NNET_imp_folds_file<-paste(c(analysis_folder,"importance/","NNET_imp_vars_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,stk_comp,trg,"_",tuning,".csv"),collapse = "")
tan_imp_folds_file<-paste(c(analysis_folder,"importance/","tan_imp_vars_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,stk_comp,trg,"_",tuning,".csv"),collapse = "")

log_perf_folds_file<-paste(c(analysis_folder,"performance/","log_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",stk_comp,trg,"_",tuning,".csv"),collapse = "")
svm_perf_folds_file<-paste(c(analysis_folder,"performance/","svm_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",stk_comp,trg,"_",tuning,".csv"),collapse = "")
CRT_perf_folds_file<-paste(c(analysis_folder,"performance/","CRT_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",stk_comp,trg,"_",tuning,".csv"),collapse = "")
NNET_perf_folds_file<-paste(c(analysis_folder,"performance/","NNET_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",stk_comp,trg,"_",tuning,".csv"),collapse = "")
tan_perf_folds_file<-paste(c(analysis_folder,"performance/","tan_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",stk_comp,trg,"_",tuning,".csv"),collapse = "")


Avg_log_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_log_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",itself_run,stk_comp,trg,"_",tuning,".csv"),collapse = "")
Avg_svm_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_svm_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",itself_run,stk_comp,trg,"_",tuning,".csv"),collapse = "")
Avg_CRT_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_CRT_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",itself_run,stk_comp,trg,"_",tuning,".csv"),collapse = "")
Avg_NNET_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_NNET_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",itself_run,stk_comp,trg,"_",tuning,".csv"),collapse = "")
Avg_tan_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_tan_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_multi,"_","_",is_bal,"_",itself_run,stk_comp,trg,"_",tuning,".csv"),collapse = "")

itself_svm_Location<-paste(c(root_folder,"itself_svm_",is_bal,"_",is_multi,stk_comp,trg,"_",tuning,"_variables.csv"),collapse = "")
itself_log_Location<-paste(c(root_folder,"itself_log_",is_bal,"_",is_multi,stk_comp,trg,"_",tuning,"_variables.csv"),collapse = "")
itself_CRT_Location<-paste(c(root_folder,"itself_CRT_",is_bal,"_",is_multi,stk_comp,trg,"_",tuning,"_variables.csv"),collapse = "")
itself_NNET_Location<-paste(c(root_folder,"itself_NNET_",is_bal,"_",is_multi,stk_comp,trg,"_",tuning,"_variables.csv"),collapse = "")
itself_tan_Location<-paste(c(root_folder,"itself_tan_",is_bal,"_",is_multi,stk_comp,trg,"_",tuning,"_variables.csv"),collapse = "")
...
#_______________________________Required Libraries_______________________________________________________________________________________
  ...
  #if(input_object$parameters["mac","content"]=="Y"){library(doMC)}
  set.seed(110)
...
VarsData <- as.data.frame(colnames(Data))  
VarsData<-as.data.frame(VarsData[-grep("TARGET", VarsData[,1]),])
...
vars<-VarsData_Phase$variables

trg<-input_object$parameters["trg","content"]

Try_data<-Data[, which(names(Data) %in% vars)]
Try_data$TARGET<-Data[,as.character(trg)]
...
VarsData_Phase$variables<-toupper(VarsData_Phase$variables)
