...
initialization<-function(input_object){
  ...
  lit_vars<-input_object$parameters$lit_vars
  ...
  trg<-input_object$parameters$target
  file_name<-input_object$parameters$file_name
  conso_feat<-input_object$parameters$conso_feat

  #__________________________________Path Initialization_____________________________
  if(platform=="MAC OS"){root_folder<-"/Users/hamid/Dropbox/My R codes/Bin-Hamid/Stock_Prob/data/"}else{root_folder<-"C:/Users/hza0020/Dropbox/My R codes/Bin-Hamid/Stock_Prob/data/"}
  if(platform=="MAC OS"){analysis_folder<-"/Users/hamid/Dropbox/My R codes/Bin-Hamid/Stock_Prob/analysis/"}else{analysis_folder<-"C:/Users/hza0020/Dropbox/My R codes/Bin-Hamid/Stock_Prob/analysis/"}
  ...
  if(input_object$parameters$tuning=="Yes"){tuning<-"tuned"}

  method_fs<-paste(input_object$parameters$FS_Method, collapse = "_")
  ...
  FS_Method_output<-""
  if(RF_run=="Yes"){FS_Method_output<-paste("Random Forest",FS_Method_output,sep="")}
  if(Lasso_run=="Yes"){FS_Method_output<-paste("Lasso",FS_Method_output,sep=" & ")}
  if(FCBF_run=="Yes"){FS_Method_output<-paste("Fast Feature Selection",FS_Method_output,sep=" & ")}
  if(itself_run=="Yes"){FS_Method_output<-paste("Same Machine Learning Algorithm for Prediction and Feature Selection",FS_Method_output,sep=" & ")}
  if(!is.null(lit_vars)){FS_Method_output<-paste("Literature",FS_Method_output,sep=" & ")}

  settings<-c(root_folder,analysis_folder,is_multi,file_name,
  ...
  row.names(settings)<-c("root_folder","analysis_folder","is_multi","file_name",
                         "method_fs","is_bal","Random_Forrest_Location","Lasso_location","consolidate_Location",
                         "Fast_Feature_Location","base_file","Out_put_file",
                         "Literature_file","prediction_data","consolidate",
                         "log_imp_folds_file","svm_imp_folds_file","CRT_imp_folds_file","NNET_imp_folds_file","tan_imp_folds_file",
