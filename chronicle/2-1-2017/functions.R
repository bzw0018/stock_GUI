...
reading <- function(inputss) {
  ...
  datapath<-inputss$datapath
  input_file_format<-inputss$input_file_format
  ...
  if(regexpr(pattern ='Literature',method_fs)[1]>-1){
    if(length(inputss$lit_vars)>0){
      lit_vars<-inputss$lit_vars}else{lit_var_nul<-"nul"
                                      method_fs<-gsub("_Literature","",method_fs)
      }}
  ...
  if(tuning_input=="Yes"){tuning<-"tuned"}else{tuning<-"not_tuned"}
  ...
  input_object$data_user<-inputss$data_user
  if(lit_var_nul!="nul"){
    input_object$data_lit<-inputss$data_lit
    
    input_object$dataset<-cbind(input_object$data_user,input_object$data_lit)
    input_object$dataset <- input_object$dataset[, !duplicated(colnames(input_object$dataset))]
    }else{input_object$dataset<-input_object$data_user}
  
  input_object$vars_independant<-uservars
  input_object$lit<-lit_vars
  email<-inputss$email
  
  params<-c(platform,NA_cleaning,Multi_Bino,is_multi,method_fs,lit_run,lit_var_nul, FS_Method_RF,FS_Method_Lasso,
            FS_Method_FCBF,FS_Method_itself, FS_Method_lit,d_prep, Lasso_run,FCBF_run,itself_run,RF_run,
            n_folds,balanc_proc,B_alg,H_por_low,H_por_high,file_name,conso_feat,root_folder,
            analysis_folder,target,tuning_input,tuning,consolidate,is_bal,email,datapath,input_file_format)
  
  params<-as.data.frame(params)
  params[]<- lapply(params, as.character)
  row.names(params)<-c("platform","NA_cleaning","Multi_Bino","is_multi","method_fs","lit_run","lit_var_nul","FS_Method_RF","FS_Method_Lasso",
                       "FS_Method_FCBF","FS_Method_itself","FS_Method_lit","d_prep","Lasso_run","FCBF_run","itself_run","RF_run",
                       "n_folds","balanc_proc","B_alg","H_por_low","H_por_high","file_name","conso_feat","root_folder",
                       "analysis_folder","target","tuning_input","tuning","consolidate","is_bal","email","datapath","input_file_format")
...
initialization<-function(input_object){
  ...
  Random_Forrest_Location<-paste(c(root_folder,"/","Random_Forrest_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  Lasso_location<-paste(c(root_folder,"/","Lasso_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  Fast_Feature_Location<-paste(c(root_folder,"/","Fast_Feature_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  Literature_file<-paste(c(root_folder,"/","Literature_",trg,"_",d_prep,"_Litrature_Review_",file_name),collapse = "")
  consolidate_Location<-paste(c(root_folder,"/","consolidate_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  removed_vars_Location<-paste(c(root_folder,"/","removed_vars_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  
  base_file<-paste(c(root_folder,"/",file_name),collapse = "")
  Out_put_file_name<-paste(c(file_name,"_",trg,"_",d_prep,"_",method_fs,"_",consolidate,"_",is_bal,"_",is_multi,"_",tuning,".txt"),collapse = "")
  Out_put_file<-paste(c(analysis_folder,file_name,"_",trg,"_",d_prep,"_",method_fs,"_",consolidate,"_",is_bal,"_",is_multi,"_",tuning,".txt"),collapse = "")
  
  prediction_data<-paste(c(root_folder,"/","prediction_data","_",method_fs,"_",consolidate,"_","_",is_multi,"_",is_bal,"_",file_name,"_",trg,"_",d_prep,"_",tuning,".csv"),collapse = "")
  
  dir.create(paste(c(analysis_folder,"importance"),collapse = ""))
  dir.create(paste(c(analysis_folder,"performance"),collapse = ""))
  
  log_imp_folds_file<-paste(c(analysis_folder,"importance/","log_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  svm_imp_folds_file<-paste(c(analysis_folder,"importance/","svm_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  CRT_imp_folds_file<-paste(c(analysis_folder,"importance/","CRT_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  NNET_imp_folds_file<-paste(c(analysis_folder,"importance/","NNET_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  tan_imp_folds_file<-paste(c(analysis_folder,"importance/","tan_imp_vars_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  
  log_perf_folds_file<-paste(c(analysis_folder,"performance/","log_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  svm_perf_folds_file<-paste(c(analysis_folder,"performance/","svm_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  CRT_perf_folds_file<-paste(c(analysis_folder,"performance/","CRT_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  NNET_perf_folds_file<-paste(c(analysis_folder,"performance/","NNET_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  tan_perf_folds_file<-paste(c(analysis_folder,"performance/","tan_perf_folds_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  
  Avg_log_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_log_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  Avg_svm_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_svm_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  Avg_CRT_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_CRT_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  Avg_NNET_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_NNET_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  Avg_tan_perf_folds_file<-paste(c(analysis_folder,"performance/","Avg_tan_perf_folds_","_",is_multi,"_",method_fs,"_",consolidate,"_",is_bal,"_",trg,"_",d_prep,"_",tuning,"_",file_name),collapse = "")
  
  itself_svm_Location<-paste(c(root_folder,"/","itself_svm_variables_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  itself_log_Location<-paste(c(root_folder,"/","itself_log_variables_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  itself_CRT_Location<-paste(c(root_folder,"/","itself_CRT_variables_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  itself_NNET_Location<-paste(c(root_folder,"/","itself_NNET_variables_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  itself_tan_Location<-paste(c(root_folder,"/","itself_tan_variables_",is_multi,trg,"_",d_prep,"_variables_",file_name),collapse = "")
  ...
  settings<-c(Random_Forrest_Location,Lasso_location,consolidate_Location,
              ...
              Literature_file,prediction_data,consolidate, removed_vars_Location,
...
Random_Forrest<-function(input_object){
  ...
  if(as.numeric(input_object$parameters["FS_Method_RF","content"]>0)){
    if(input_object$parameters["RF_run","content"]=="No"){
      ...
      input_object$Random_Forrest<-R_Forrest
      
      write.csv(input_object$Random_Forrest,input_object$settings["Random_Forrest_Location","content"])
    }else{
      Random_Forrest<-read.csv(input_object$settings["Random_Forrest_Location","content"])
      R_Forrest<-as.data.frame(Random_Forrest$variables)
      colnames(R_Forrest)<-c("variables")
      input_object$Random_Forrest<-R_Forrest}
  }
  return(input_object)}
...
Lasso_Multinomial<-function(input_object){
  ...
  if(input_object$parameters["Multi_Bino","content"]=="Multinomial"){
    if(as.numeric(input_object$parameters["FS_Method_Lasso","content"])>0){
      if(input_object$parameters["Lasso_run","content"]=="No"){
        ...
        Lasso_imp_cleaned$version<-as.character(Lasso_imp_cleaned$version)
        Lasso_temp<-as.data.frame(Lasso_imp_cleaned$variables)
        colnames(Lasso_temp)<-c("variables")
        input_object$Lasso<-Lasso_temp
        
        write.csv(Lasso_temp,input_object$settings["Lasso_location","content"])}
      else{temp_Lasso<-read.csv(input_object$settings["Lasso_location","content"])
      Lasso_temp<-as.data.frame(temp_Lasso$variables)
      colnames(Lasso_temp)<-c("variables")
      input_object$Lasso<-Lasso_temp}
    }}
  return(input_object)
...
Lasso_Binomial<-function(input_object){
  ...
  if(input_object$parameters["Multi_Bino","content"]=="Binomial"){
    if(as.numeric(input_object$parameters["FS_Method_Lasso","content"])>0){
      if(input_object$parameters["Lasso_run","content"]=="No"){
...
FFS<-function(input_object){
  ...
  if(as.numeric(input_object$parameters["FS_Method_FCBF","content"])>0){
    ...
    if(input_object$parameters["FCBF_run","content"]=="No"){
      library(Biocomb)
      disc<-"MDL"
      threshold=0
      ...
      input_object$FFS<-FFS
      
      write.csv(FFS,input_object$settings["Fast_Feature_Location","content"])}
    
    else{FF_vars<-read.csv(input_object$settings["Fast_Feature_Location","content"])
    FFS<-as.data.frame(FF_vars$variables)
    colnames(FFS)<-c("variables")
    input_object$FFS<-FFS}
  }
...
itself<-function(input_object){
  if(as.numeric(input_object$parameters["FS_Method_itself","content"]>0)){
    ...
    if(input_object$parameters["itself_run","content"]=="No"){
      ...
      NNET_Profile <- rfe(x, TARGET,
                          ...
                          metric="ROC",
                          trControl=trainctrl)
      if(input_object$parameters["platform","content"]=="MAC OS"){registerDoMC(cores = NULL)}
      
      vars<-as.data.frame(colnames(input_object$dataset))
      
      it_svm_tmp<-var_finder(vars,as.data.frame(svm_Profile$optVariables))
      svm_it<-as.data.frame(it_svm_tmp$variables)
      colnames(svm_it)<-c("variables")
      input_object$itself$svm<-svm_it
      
      it_log_tmp<-var_finder(vars,as.data.frame(log_Profile$optVariables))
      log_it<-as.data.frame(it_log_tmp$variables)
      colnames(log_it)<-c("variables")
      input_object$itself$log<-log_it
      
      it_crt_tmp<-var_finder(vars,as.data.frame(CRT_Profile$optVariables))
      crt_it<-as.data.frame(it_crt_tmp$variables)
      colnames(crt_it)<-c("variables")
      input_object$itself$crt<-crt_it
      
      it_tan_tmp<-var_finder(vars,as.data.frame(TAN_Profile$optVariables))
      tan_it<-as.data.frame(it_tan_tmp$variables)
      colnames(tan_it)<-c("variables")
      input_object$itself$tan<-tan_it
      
      it_nnet_tmp<-var_finder(vars,as.data.frame(NNET_Profile$optVariables))
      nnet_it<-as.data.frame(it_nnet_tmp$variables)
      colnames(nnet_it)<-c("variables")
      input_object$itself$nnet<-nnet_it
      ...
      svm_tt<-read.csv(input_object$settings["itself_svm_Location","content"])
      tt_svm<-as.data.frame(svm_tt$variables)
      colnames(tt_svm)<-c("variables")
      input_object$itself$svm<-tt_svm
      
      log_tt<-read.csv(input_object$settings["itself_log_Location","content"])
      tt_log<-as.data.frame(log_tt$variables)
      colnames(tt_log)<-c("variables")
      input_object$itself$log<-tt_log
      
      crt_tt<-read.csv(input_object$settings["itself_CRT_Location","content"])
      tt_crt<-as.data.frame(crt_tt$variables)
      colnames(tt_crt)<-c("variables")
      input_object$itself$crt<-tt_crt
      
      tan_tt<-read.csv(input_object$settings["itself_tan_Location","content"])
      tt_tan<-as.data.frame(tan_tt$variables)
      colnames(tt_tan)<-c("variables")
      input_object$itself$tan<-tt_tan
      
      nnet_tt<-read.csv(input_object$settings["itself_NNET_Location","content"])
      tt_nnet<-as.data.frame(nnet_tt$variables)
      colnames(tt_nnet)<-c("variables")
      input_object$itself$nnet<-tt_nnet
...
#=============================================================
Lit<-function(input_object){
  if(as.numeric(input_object$parameters["FS_Method_lit","content"]>0)){if(input_object$parameters["lit_var_nul","content"]!="nul"){
    lit_vars<-as.data.frame(input_object$lit)
    colnames(lit_vars)<-c("variables")
    input_object$lit<-lit_vars}}
  return(input_object)
}
...
consolidate<-function(input_object){
  ...
  template<-data.frame(variables=character())
  consolidated<-template
  
  Random_Forrest<-input_object$Random_Forrest
  Lasso<-input_object$Lasso
  FFS<-input_object$FFS
  itself_svm<-input_object$itself$svm
  itself_log<-input_object$itself$log
  itself_crt<-input_object$itself$crt
  itself_tan<-input_object$itself$tan
  itself_nnet<-input_object$itself$nnet
  if(input_object$parameters["lit_var_nul","content"]!="nul"){
    lit<-input_object$lit}
  
  #check if below mentioned variables exist
  method_fs<-input_object$parameters["method_fs","content"]
  
  if(regexpr(pattern ='Random Forrest',method_fs)>-1){
    if(!is.null(Random_Forrest)){
      if(nrow(input_object$Random_Forrest)>0){consolidated<-rbind(consolidated,input_object$Random_Forrest)}}}
  
  if(regexpr(pattern ='Lasso',method_fs)>-1){
    if(!is.null(Lasso)){
      if(nrow(input_object$Lasso)>0){consolidated<-rbind(consolidated,input_object$Lasso)}}}
  
  if(regexpr(pattern ='Fast Selection',method_fs)>-1){
    if(!is.null(FFS)){
      if(nrow(input_object$FFS)){consolidated<-rbind(consolidated,input_object$FFS)}}}
  
  if(regexpr(pattern ='itself',method_fs)>-1){
    
    if(!is.null(itself_svm)){
      if(nrow(input_object$itself$svm)>0){consolidated<-rbind(consolidated,input_object$itself$svm)}}
    
    if(!is.null(itself_log)){
      if(nrow(input_object$itself$log)>0){consolidated<-rbind(consolidated,input_object$itself$log)}}
    
    if(!is.null(itself_crt)){
      if(nrow(input_object$itself$crt)>0){consolidated<-rbind(consolidated,input_object$itself$crt)}}
    
    if(!is.null(itself_tan)){
      if(nrow(input_object$itself$tan)>0){consolidated<-rbind(consolidated,input_object$itself$tan)}}
    
    if(!is.null(itself_nnet)){
      if(nrow(input_object$itself$nnet)>0){consolidated<-rbind(consolidated,input_object$itself$nnet)}}}
  
  if(regexpr(pattern ='Literature',method_fs)>-1){
    if(input_object$parameters["lit_var_nul","content"]!="nul"){
      if(!is.null(lit)){
        if(nrow(input_object$lit)>0){consolidated<-rbind(consolidated,input_object$lit)}}}}
  
  consolidated<-as.data.frame(consolidated[!duplicated(consolidated$variables), ])
  names(consolidated)<-"variables"
  
  input_object$consolidated<-consolidated
  write.csv(input_object$consolidated,input_object$settings["consolidate_Location","content"])
...
remove_conso<-function(input_object){
  if(input_object$parameters["d_prep","content"]=="DP"){
    ...
    names(remove_conso)<-"variables"
    
    invariant_data<-input_object$remove$invariant_data
    too_levels<-input_object$remove$too_levels
    ...
    if(!is.null(invariant_data)){
      ...
      remove_conso<-cbind(remove_conso,VarsData_inv)
    }
    
    if(!is.null(too_levels)){
      VarsData_lvl<- as.data.frame(colnames(input_object$remove$too_levels))
      names(VarsData_lvl)<-"variables"
      remove_conso<-cbind(remove_conso,VarsData_lvl)
    }
    ...
    if(nrow(remove_conso)>1)
      input_object$remove$removed_vars<-remove_conso
    write.csv(input_object$remove$removed_vars,input_object$settings["removed_vars_Location","content"])
  }
  return(input_object)
}
...
ADP<-function(input_object){
  if(input_object$parameters["d_prep","content"]=="DP"){
    ...
    if(length(invariant_index)>0){
      ...
      invariant_data<-data[, invariant_index]
    }
    if(length(invariant_index)>0){input_object$remove$invariant_data<-invariant_data}
    
    too_levels<-data.frame(variables=character())
    #dropping variables with too many categories
    for(i in 1:ncol(input_object$dataset)){
      if(is.factor(input_object$dataset[,i])){
        if(nlevels(input_object$dataset[,i])>10){
          too_levels<-rbind(too_levels,data[,i])
          {data[,i]<-NULL}}
        
      }}
    if(ncol(data)==ncol(input_object$dataset)){
      if(length(too_levels)>0){input_object$remove$too_levels<-too_levels}}
    ...
    if(exists(deparse(substitute(nzvar)))){
    ...
    if(exists(deparse(substitute(non_numerics)))){
      if(length(non_numerics)>0){non_numeric_vars<-non_numerics[,1]
      data_non_numerics<-ddd[, which(names(ddd) %in% non_numeric_vars)]
      if(nrow(non_numerics)>0){data_non_numerics<-as.data.frame(data_non_numerics)
      names(data_non_numerics)<-as.character(non_numeric_vars)}}}
    
    if(exists(deparse(substitute(numerics)))){
      if(length(numerics)>0){numeric_vars<-numerics[,1]
      ...
      if(length(highlyCorDescr)>0){
        ...
        high_corr<-data_numerics[, highlyCorDescr]
        input_object$remove$high_corr<-high_corr
      }}}
    
    if(exists(deparse(substitute(data_non_numerics)))){if(exists(deparse(substitute(data_numerics)))){
      if(length(data_non_numerics)>0){if(length(data_numerics)>0){
        if(length(data_numerics)>0){data<-cbind(data_non_numerics,data_numerics)}else{data<-data_non_numerics} }else{if(length(data_numerics)>0){data<-data_numerics}}}}}
    ...
    # if(input_object$parameters["lit_var_nul","content"]!="kk"){
    #    data<-merge(data,input_object$data_lit)
    #  }
...
multi_pred<-function(input_object){
  if(input_object$parameters["Multi_Bino","content"]=="Multinomial"){
    ...
    control_<- trainControl(method="cv",number = as.numeric(input_object$parameters["n_folds","content"]) ,savePredictions = TRUE,
...
Bino_pred<-function(input_object){
  if(input_object$parameters["Multi_Bino","content"]=="Binomial"){
    ...
    if(input_object$parameters["lit_var_nul","content"]!="nul"){
      data<-cbind(data,input_object$data_lit)
    ...
    if(conso_feat=="Yes"){
      consolidated_vars<-input_object$consolidated
      vars<-as.character(consolidated_vars$variables)
      # if(!exists(deparse(substitute(input_object$lit)))){
    ...
    for ( i in 1: n_folds){
      ...
      if(B_alg=="Hybrid"){
       ...
       # table(train_data$TARGET)
            ...
            if(conso_feat=="No"){formula<-formula_svm}
            mod_svm<-train(formula,  data=train_data, method="svmRadial", family="binomial",
                           trControl = control_,  tuneGrid=expand.grid(C=svm_cost, sigma=svm_sigma),
                           tuneLength = 10)

            if(conso_feat=="No"){formula<-formula_NNET}
